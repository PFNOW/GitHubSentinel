...
from hacker_news_client import HackerNewsClient  # 从hacker_news_client模块导入HackerNewsClient类，用于Hacker News API操作
...
from command_handler import GithubCommandHandler,HackerNewsCommandHandler  # 从command_handler模块导入CommandHandler类，处理命令行命令
...
def main():
    ...
    while True: # 启用项目
        start_program = input("GitHub Sentinel or Hacker News Sentinel? (gh/hn) ")
        if start_program == "gh":
    ...
    while True: # 循环执行命令
        try:
            user_input = input(f"{start_program}> ")  # 等待用户输入
